...
const Homevegetableshop = () => {
  const [products, setProducts] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/getAllvegetableshop");
              ...
              {products.map((product, index) => {
                return (
                  <tr key={product._id}>
                    ...
                    <td>
                      <Link to={`/viewvegetableshop/` + product._id}>
                        <i
                      ...
                      <Link
                        to={`/editvegetableshop/` + product._id}
                        className="mx-2"
                      >
                        <i
...
export default Homevegetableshop;